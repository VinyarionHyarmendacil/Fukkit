...
public class FMarkOfMaker extends FAnvilRecipe implements Permenant {

	public static void init() {
		FAnvilRecipes.instance().addPerm(new FMarkOfMaker("MarkOfMaker_engrave", false));
	}

	public FMarkOfMaker(String name, boolean pub) {
		super(name, pub);
	}

	public boolean matches(ItemStack left, ItemStack top, ItemStack bottom) {
		return left != null && (top != null && top.getItem() == LOTRMod.blacksmithHammer) && (bottom != null && bottom.getItem() == LOTRMod.chisel);
	}
	...
	public ItemStack clickResult(LOTRContainerAnvil anvil, ItemStack result) {
		...
		slots[1].getStack().attemptDamageItem(3, FRingForging.rr);
		slots[2].getStack().attemptDamageItem(3, FRingForging.rr);
